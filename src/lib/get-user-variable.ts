...
function getKey(key: User | Record<string, primitive>, property: string): UserVariable {
  return key[property] || null;
}

export default function getUserVariable(user: User, property: string, selectedApp?: primitive): UserVariable {
  let key = user;
