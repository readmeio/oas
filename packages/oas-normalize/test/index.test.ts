...
describe('OASNormalize', () => {
  ...
  describe('.bundle()', () => {
    ...
    describe('orphaned `$id` keywords', () => {
      it('should bundle a schema whose internal `$ref` is mis-scoped by a sibling `$id`', async () => {
        ...
        const bundled = (await o.bundle()) as OpenAPIV3.Document;
        const pets = bundled.components?.schemas?.Pets as Record<string, unknown>;

        expect(pets).not.toHaveProperty('$id');
        expect(pets.items).toStrictEqual({
          $ref: '#/paths/~1petsl/get/responses/200/content/application~1json/schema',
        });
      });

      it('should bundle when a `$id` mis-scopes a pointer between two component schemas', async () => {
        const o = new OASNormalize({
          openapi: '3.1.0',
          info: { title: 't', version: '1' },
          ...
          components: {
            schemas: {
              Wrapper: { $id: 'wrapper.json', allOf: [{ $ref: '#/components/schemas/Real' }] },
              Real: { type: 'object', properties: { id: { type: 'integer' } } },
            },
          },
        });

        const bundled = (await o.bundle()) as OpenAPIV3.Document;
        const wrapper = bundled.components?.schemas?.Wrapper as Record<string, unknown>;

        expect(wrapper).not.toHaveProperty('$id');
        expect(wrapper.allOf).toStrictEqual([{ $ref: '#/components/schemas/Real' }]);
      });

      /**
       * Guard against the fix over-stripping: a `$id` that a `$ref` actually targets is a legitimate
       * reference anchor and must be preserved. This already bundles today; it should keep doing so.
       */
      it('should preserve a `$id` that is used as a legitimate reference anchor', async () => {
        const o = new OASNormalize({
          openapi: '3.1.0',
          info: { title: 't', version: '1' },
          paths: {
            '/order': {
              get: {
                responses: {
                  '200': {
                    description: 'ok',
                    content: { 'application/json': { schema: { $ref: 'https://ex.com/address.json' } } },
                  },
                },
              },
            },
        ...
        expect(bundled.components?.schemas?.Address).toHaveProperty('$id', 'https://ex.com/address.json');
      });
    });
  });
