...
describe('Invalid APIs (Swagger 2.0 and OpenAPI 3.x schema validation)', () => {
  it('should pass validation if "options.validate.schema" is false', async () => {
    const api = await SwaggerParser.validate(path.rel('specs/validate-schema/invalid/invalid-response-code.yaml'), {
  ...
  it('should return all errors', async () => {
    try {
      await SwaggerParser.validate(path.rel('specs/validate-schema/invalid/multiple-invalid-properties.yaml'));
  ...
  it.each([
    ...
    const api = await SwaggerParser.validate(path.rel(`specs/validate-schema/valid/${file}`));
    expect(api).to.be.an('object');
  });
  ...
  it.each([
    ...
    try {
      await SwaggerParser.validate(path.rel(`specs/validate-schema/invalid/${file}`));
      assert.fail('Validation should have failed, but it succeeded!');
    } catch (err) {
      expect(err).to.be.an.instanceOf(SyntaxError);
      if (isOpenAPI) {
