...
import operationServerPrecedenceSpec from './__datasets__/operation-server-precedence.json' with { type: 'json' };
...
describe('Oas', () => {
  ...
  describe('Operation server helpers', () => {
    it('should fall back to root-level servers', () => {
      const oas = Oas.init(operationServerPrecedenceSpec);
      const operation = oas.operation('/root-server', 'get');

      expect(operation.getServers()).toStrictEqual(operationServerPrecedenceSpec.servers);
      expect(operation.url()).toBe('https://us.root.example.com/v1');
    });

    it('should prefer path-item servers over root-level servers', () => {
      const oas = Oas.init(operationServerPrecedenceSpec);
      const operation = oas.operation('/path-server', 'get');
      ...
      expect(operation.url()).toBe('https://eu.path.example.com/v2');
      expect(operation.defaultVariables()).toStrictEqual({ region: 'eu', version: 'v2' });
    ...
    it('should prefer operation servers over path-item and root-level servers', () => {
      const oas = Oas.init(operationServerPrecedenceSpec);
      const operation = oas.operation('/operation-server', 'get');

      expect(operation.getServers()).toStrictEqual([
        {
          url: 'https://{region}.operation.example.com/{version}',
          variables: {
            region: { default: 'apac', enum: ['apac', 'emea'] },
            version: { default: 'v3' },
          },
        },
      ]);
      expect(operation.url()).toBe('https://apac.operation.example.com/v3');
    ...
    it('should embellish operation server defaults with user variables', () => {
      const oas = new Oas(operationServerPrecedenceSpec, { region: 'emea' });
      const operation = oas.operation('/operation-server', 'get');

      expect(operation.defaultVariables()).toStrictEqual({ region: 'emea', version: 'v3' });
      expect(operation.url()).toBe('https://emea.operation.example.com/v3');
    });

    it('should use the selected operation server and its variables', () => {
      const oas = Oas.init(operationServerPrecedenceSpec);
      const operation = oas.operation('/multiple-operation-servers', 'get');

      expect(operation.url()).toBe('https://first.operation.example.com');
      expect(operation.url(1)).toBe('https://us.second.operation.example.com/v4');
      expect(operation.defaultVariables(1)).toStrictEqual({ region: 'us', version: 'v4' });
    });
    ...
    it('should ignore empty operation and path-item server arrays when applying precedence', () => {
      ...
      expect(oas.operation('/empty-path-item-servers', 'get').url()).toBe('https://us.root.example.com/v1');
    });
    ...
    it('should fall back to the default server URL when the selected server is unavailable', () => {
      ...
      expect(operation.url(1)).toBe('https://example.com');
