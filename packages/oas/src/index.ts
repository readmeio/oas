...
export default class Oas {
  ...
  operation(
    ...
    if (opts.isWebhook) {
      if (isOpenAPI31(this.api)) {
        ...
        if (!isRef(webhookPath) && webhookPath?.[method]) {
  ...
  getPaths(): Record<string, Record<HttpMethods, Operation | Webhook>> {
    ...
    Object.keys(this.api.paths).forEach(path => {
      ...
      if (pathItem) {
        // biome-ignore lint/style/noNonNullAssertion: We're guaranteed to have `api.paths[path]` from the `.keys()` loop.
        this.api.paths![path] = dereferenceRef(pathItem, this.api);

        // biome-ignore lint/style/noNonNullAssertion: We're guaranteed to have `api.paths[path]` from the `.keys()` loop.
        Object.keys(this.api.paths![path]).forEach(method => {
          /**
           * Because a path doesn't need to contain a keyed-object of HTTP methods, we should
           * exclude anything from within the paths object that isn't a known HTTP method.
           *
